...
public class Mechamoo extends Monster implements GeoEntity
{
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    public static final EntityDataAccessor<Integer> ATTACK = SynchedEntityData.defineId(Mechamoo.class, EntityDataSerializers.INT);

    public Mechamoo(EntityType<? extends Monster> entityType, Level level) { super(entityType, level); }
    ...
    {
        this.playSound(SoundEvents.SHULKER_AMBIENT, 1.0F, 0.2F);
        return null;
    }
    ...
    {
        this.playSound(SoundEvents.ANVIL_PLACE, 1.0F, 0.2F);
        return null;
    }
    ...
    @Override
    protected float getStandingEyeHeight(Pose poseIn, EntityDimensions sizeIn) { return 4.35F; }
    ...
    public void performRangedAttack(LivingEntity livingEntity, float p_32142_)
    {
        ...
        double d0 = livingEntity.getEyeY() - (double)3.7F;
        double d1 = livingEntity.getX() - this.getX();
        double d2 = d0 - arrow.getY();
        double d3 = livingEntity.getZ() - this.getZ();
        double d4 = Math.sqrt(d1 * d1 + d3 * d3) * (double)0.2F;
        arrow.shoot(d1, d2 + d4, d3, 3.5F, 5.5F);
        this.playSound(SoundEventsROTM.MECHAMOO_SHOT.get(), 5.0F, 1.0F / (this.getRandom().nextFloat() * 0.6F + 1.2F));
